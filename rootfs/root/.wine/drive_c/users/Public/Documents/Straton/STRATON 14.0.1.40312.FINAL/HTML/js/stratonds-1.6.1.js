 ...
 * stratonds-1.6.1.js
 * 
 * - remove _addDebugInfoInCmd and all the referenes to currentTestingMode
 *
 *
 * stratonds-1.6.js
 * 
...
function dsFetchError(message,error,errorTxt) {
...
async function _fetchCmd(fetchData,cbOK,cbFailed, generalAbortSignal)
{
    ...
    const response = await fetch(fetchData.urlquery, initObject)
        ...
        .then(function (response) {
            if (!response.ok) {
                throw new dsFetchError("Request - Network error " + response.status + " - " + response.statusText,response.status,response.statusText);
              }
        ...
        .then(function (data) {
			...
			//Add cmdExtData if defined (used for auto test - cmdExtData is defined by the caller
			if (typeof cmdExtData !== "undefined") 
			{
				data.cmdExtData = cmdExtData;
			}
			return data;
        })
        .catch(function (err) {
